  ...
  <div class="home">
    <div class="content">
      ...
      <h3 class="row gx-3 gy-2 align-items-center">
        <center>ลอตเตอรี่ออนไลน์</center>
      </h3>
      <p class="row gx-3 gy-2 align-items-center text-danger">
        <center>ลอตเตอรี่งวดวันที่ {{ deadlinedate }}</center>
      </p>
      <h3 class="row gx-3 gy-2 align-items-center">
...
<div class="row">
        <div v-for="lotto in info" :key="lotto.f_code" class="col-md-4">
          <h3 class="product__header">{{ lotto.f_number }}</h3>
          <img v-bind:src="'/' + lotto.f_pic" :alt="lotto.f_number" class="product_image"/> 
          <p class="product__description">{{ lotto.f_barcode }}</p>
          <div class="cart">
            <button
              @click="updateCart(lotto, 'subtract')"
              class="cart__button"
            >
              -
            </button>
            <span class="cart__quantity">{{ lotto.f_price }}</span>
            <button @click="updateCart(lotto, 'add')" class="cart__button">
              +
            </button>
          </div>
        </div>
